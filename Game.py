...
class MainApp(App):
	def build(self,**args):
		...
		self.button1 = Button(text='Об авторе \n        и \n этой игре', size_hint=(1, 0.075), pos_hint={'top': 0})
		self.button2 = Button(text='Выход', size_hint=(1, 0.075), pos_hint={'top': 0},on_press=self.out)
		...
		blb.add_widget(self.button1)
		blb.add_widget(self.button2)
	...
	def out(self,*args):
		#self.main_secret_text.text = '?txt?  some simbols in asci mb  1212212121   '
		old = self.build
		self.button1.text = ''
		bl.remove_widget(blb)
		bl.remove_widget(fl)

		but = Button(text='quyt')
		MainApp().stop()
		bl.add_widget(but)
		return bl
